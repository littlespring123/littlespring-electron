<template>
	<form ref="form" class="form" @submit.prevent="handleSubmit">
		<div v-for="field in inFormFields" :key="field.name" class="item">
			<Input v-model="formData[field.name]" :label="field.label" :type="field.type" :name="field.name" :placeholder="field.placeholder" />
			<span :class="field.error ? 'error' : 'hiden-error'"> {{ field.error }}</span>
		</div>
		<div class="footer">
			<Button type="submit" @click="opearteSubmmit">{{ t("form.confirm") }}</Button>
